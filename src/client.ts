...
import { Routing } from "./resources/routing.js";
import { Runtime } from "./resources/runtime.js";
import type { Schedules } from "./resources/schedules.js";
...
export class Sonzai {
	...
	/**
	 * Legacy platform-hosted built-in-agent execution surface. New runtimes
	 * download definitions through `runtime.backendAgentArtifacts()` and execute
	 * them locally with their configured provider.
	...
	/**
	 * Project-scoped workflow definitions. Runtime deployments execute these
	 * locally; legacy platform run helpers remain for source compatibility.
	 */
	readonly pipelines: Pipelines;
	/** Project routing tiers, guide/handoff rules, and permanent contact routes. */
	readonly routing: Routing;
	/**
	 * Stable control-plane contract for custom runtimes. Fetches context and
	 * reports turns/usage; LLM provider calls execute in the runtime itself.
	 */
	...
	constructor(config?: SonzaiConfig) {
		...
		this.routing = new Routing(this.http);
		this.runtime = new Runtime(this.http);
	}

	/**
	 * Call any platform API operation through the SDK transport. Prefer a
	 * named resource when one exists; this escape hatch keeps custom runtimes
	 * compatible with every OpenAPI operation between convenience releases.
	 */
	request<T = unknown>(
		method: string,
		path: string,
